                ...
                (
                    ...
                    [
                        p.circle(
                            x="x",
                            y="y",
                            source=source.loc[source["Region"] == region],
                            size=9,
                    ...
                    (
                        ...
                        [
                            p.circle(
                                x="x",
                                y="y",
                                source=source.loc[source["fid"] == pop],
                                size=6,
                                color=colours[counter],
                                muted_alpha=0.2,
                            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Take genotype data in PLINK format, perform UMAP dimension reduction and plot the result"
    )
    parser.add_argument(
        "-i", "--input", default="pcs.txt", help="Basename of the PLINK file"
    )
    parser.add_argument("-o", "--output", default="UMAP", help="Name of outputfile")
    parser.add_argument("-k", "--key", help="File with Population legend key")
